var wysibb=wysibb||{};
wysibb.helpers={bbReplace:function(b,g,p){for(;b.match(g);)b=b.replace(g,p);return b},wbbStringFormat:function(b,g){return b.replace(/\{([\w\.]*)\}/g,function(b,s){var v=s.split("."),w=g[v.shift()];$.each(v,function(){w=w[this]});return null===w||void 0===w?"":w})},checkBBtoHTML:function(b){var g,p,s=b.bbToHTML;if(!s||this.isEmpty(s))s={},g=b.bbOpen+"(.*?)"+b.bbClose,p=b.htmlOpen+"$1"+b.htmlClose,s[g]=p,b.bbToHTML=s},replaceBBtoHTML:function(b,g){var p,s,v;for(v in g)g.hasOwnProperty(v)&&(p=g[v],
v=v.replace(/\*\]/g,"\\*]").replace(/\[/g,"\\[").replace(/\]/g,"\\]").replace(/\\\[\\\[/g,"[").replace(/\\\]\\\]/g,"]"),v=v.replace("(.*?)","([\\s\\S]*?)"),s=RegExp(v,"gmi"),b=wysibb.helpers.bbReplace(b,s,p));return b},removeBreaks:function(b){var g;g=/(\r\n|\n|\r)/gm;b=b.replace(g,"<1br />");g=/<1br \/>/gi;b=b.replace(g," ");g=/\s+/g;return b=b.replace(g," ")},isEmpty:function(b){for(var g in b)if(b.hasOwnProperty(g))return!1;return!0}};wysibb=wysibb||{};
...
!g.type||"select"!==g.type)b=N(b,g);else{v=g.options.split(",");w=v.length;for(r=0;r<w;r+=1)g=v[r],g=s.selectOptions[g],b=N(b,g)}b=$("<div>"+b+"</div>");b.find("ul > br,table > br, tr > br").each(function(b,g){$(g).remove()});return b.html()};wysibb=wysibb||{};
...
f[1];f=f.replace(/=.*/,"");var k="/"==f.substr(0,1);f.replace("/","").toLowerCase()==a.toLowerCase()&&(k?h--:h++)}return 0<h}function S(a,d,c){return q?y(d):b(c).is(a)||0<b(c).parents(a).size()}function E(a,d){d||(d=j.getRange());for(var c=j.getNodeByRange(d);c&&"BODY"!=c.tagName;){if(b(c).is(a))return c;if(c)c=c.parentNode;else break}return null}function Q(a){var b=j.getTextWithInfo(),c=g.value.substr(0,b.start),h=g.value.substr(b.end,g.value.length-b.end),f=RegExp("^(.*?)\\[\\/"+a+"\\]","i"),c=
c.replace(RegExp("\\["+a+"(?:\\=http.*?)*\\](.*?)$","i"),"$1"),h=h.replace(f,"$1");g.value=c+b.text+h}function R(a){var d="";Y(l);var a=b(a).clone(),c;for(c in i.smileList){var h=i.smileList[c],f=h.bbcode,g=b(h&&h.img?h.img:"");a.find("img").each(function(){b(this).attr("src")==g.attr("src")&&b(this).replaceWith(f)})}for(c=0;c<H.length;c++){var h=i.allButtons[H[c]].htmlToBB,j;for(j in h){var m=h[j];a.find(j).each(function(a,d){var c=m,c=c.replace(/\%(.*?)\%/g,function(a){a=a.substr(1,a.length-2);
a=a.replace("this","el");try{return eval(a)}catch(c){return b(d).html()}});b(d).replaceWith("<span>"+c+"</span>")})}}a.find("*[mustremoved='1']").remove();a.contents().each(function(){if(3===this.nodeType){var a=b(this).text();d+=a}else if(b(this)[0].tagName){if("br"==b(this)[0].tagName.toLowerCase())return d+="\n",!0;d+=R(this)}});return d}function T(a){return wysibb.render(a)}function ba(a){b(a).find("*").filter(function(){var a=b(this)[0].tagName.toLowerCase();if(-1==b.inArray(a,i.validTags))return b(this).after(this.innerHTML),
!0;var c=this,a=b.map(this.attributes,function(a){return a.name});b.each(a,function(a,d){"href"!=d&&("src"!=d&&"class"!=d)&&b(c).removeAttr(d)});return!1}).remove()}function ca(){var a=[],d=b("a img[alt=':)']"),c=b("a img[alt=';)']");if(0<d.size()&&0<c.size()){var h,f=c.parents();d.parents().each(function(a,b){f.each(function(a,d){if(b==d)return h=b,!1});if(h)return!1});b(h).find("img[alt!='']").each(function(d,c){var f=b(c).attr("title"),h=b(c).attr("alt"),g=c.outerHTML;f&&h&&a.push({title:f,img:g,
bbcode:h})})}5<a.length&&(i.smileList=a)}function da(){for(var a={},b=[],c=0;c<i.smileList.length;c++){var h=i.smileList[c];h.img=h.img.replace(h.bbcode,"");a[h.bbcode]=h;b.push(h.bbcode)}b.sort(function(a,b){return a.length>b.length?-1:1});h=[];for(c=0;c<b.length;c++)h.push(a[b[c]]);i.smileList=h}function ea(a,d){function c(){return b(this).attr("sl")==i?o=!0:0!=b(this).find('*[sl="'+i+'"]').size()?(b(this).contents().filter(c).remove(),!1):o}function h(){return b(this).attr("sl")==i?(o=!1,!0):0!=
...
"--xxxxxxxxx\r\n",k=unescape(encodeURIComponent(d.name)),a=a+("Content-Disposition: form-data; name='img'; filename='"+k+"'\r\n")+"Content-Type: application/octet-stream\r\n\r\n"+((d.getAsBinary?d.getAsBinary():d.readAsBinary())+"\r\n");f.sendAsBinary(a+"--xxxxxxxxx--")}}function m(a,b){return wysibb.helpers.wbbStringFormat(a,b)}function X(a,d){var c=b("#wbbModalWindow");0==c.size()&&(c=b(m('<div id="wbbModalWindow" style="display:none"><div class="modal-title"></div><div class="modal-info"></div><div class="modal-close"><button id="wbbCloseModal" class="wbb-redbtn" style="font-size:0.9em;width:120px;margin:5px auto">\u0417\u0430\u043a\u0440\u044b\u0442\u044c</button></div></div>')),
c.find("#wbbCloseModal").click(function(){c.hide();b(document).unbind("mousedown",V)}),b(document.body).append(c));c.find(".modal-title").html(a);c.find(".modal-info").html(d);c.css("margin-top",-1*(c.outerHeight()/2)+"px");c.css("margin-left",-1*(c.outerWidth()/2)+"px");c.css("visibility","visible").css("display","block");b(document).bind("mousedown",V)}function V(a){0==b(a.target).parents("#wbbModalWindow").size()&&(b("#wbbModalWindow").hide(),b(document).unbind("mousedown",V))}var i=wysibb.options,
W;p&&p.preset&&(b.log("Load preset: "+p.preset),b.extend(!0,i,presets[p.preset]||{}));p&&"undefined"!=typeof p.smileList&&(i.smileList=[]);b.extend(!0,i,p);b.extend(b.support,{fileSelecting:null!=window.File&&null!=window.FileList,fileReading:null!=window.FileReader,fileSending:null!=window.FormData});i.themePrefix||b("script").each(function(a,d){var c=b(d).get(0).src.match(/(.*)jquery\.wysibb(\.min)?\.js.*$/);if(c!==null)i.themePrefix=c[1]});W=/android.+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(navigator.userAgent||
...
k=k.replace(/rootNode/g,"this");b(f).find(a.rootNode).each(function(){b(this).replaceWith(eval(k))})}h=f.innerHTML;h=u(m("{htmlOpen}{txt}{htmlClose}",{htmlOpen:a.htmlOpen,htmlClose:a.htmlClose,txt:h}));c?b(c).replaceWith(h):j.overrideWithNode(h,null);b(t).attr("wbb","true");j.setSelection(null,h,0,0)}}}};this.queryState=function(a){return S(a.rootNode,a.bbName,j.getNode())}},linkCommand:function(){this.execute=function(a){if(q){g.focus();var d=a.bbName;if(y(d))Q(d);else{d=window.prompt("\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u0430\u0434\u0440\u0435\u0441 \u0441\u0441\u044b\u043b\u043a\u0438:",
...
c.focus()}});f.find("button.wbb-redbtn").click(function(){var a=j.getNodeByRange(j.getSavedRange());j.overrideWithNode(b(a).html(),j.getSavedRange(),"a");b("#wbbModalWindow").hide();l.focus()});X("\u0412\u0441\u0442\u0430\u0432\u043a\u0430 \u0441\u0441\u044b\u043b\u043a\u0438",f);f.find("#veditor_ahref").bind("keyup change",function(){(b(this).val()==""||(b(this).val().indexOf(b("#veditor_atitle").val())!=-1||b("#veditor_atitle").val().indexOf(b(this).val())!=-1)&&b(this).val().length>b("#veditor_atitle").val().length)&&
b("#veditor_atitle").val(b(this).val())}).focus()}};this.queryState=function(a){return S("a",a.bbName,j.getNode())}},imgCommand:function(a){this.execute=function(d){if(q){g.focus();var c=d.bbName;if(y(c))Q(c);else{b.isFunction(a)||(a=N);c=function(a){C(d,{src:a.url})};a({callback:c})}}else{l.focus();var h=E("img");j.saveRange();b.isFunction(a)||(a=Z);c=function(a){a=a.thumb&&d.uploader.useThumb?u(m('<a href="{img}"><img src="{thumb}" /></a>',a),t):u(m('<img src="{img}" />',a),t);j.overrideWithNode(a,
...
E(".wbb-left,.wbb-center,.wbb-right"),c=j.getHTML();if(d){a=u(m("{htmlOpen}{txt}{htmlClose}",{htmlOpen:a.htmlOpen,htmlClose:a.htmlClose,txt:b(d).html()}));b(d).replaceWith(a)}else{a=u(m("{htmlOpen}{txt}\ufeff{htmlClose}",{htmlOpen:a.htmlOpen,htmlClose:a.htmlClose,txt:c}));j.overrideWithNode(a)}j.setSelection(null,a,0,0)}}};this.queryState=function(a){return S(a.rootNode,a.bbName,j.getNode())}}};this.getBBCode=function(){return q?n.val():R(l)};this.pasteEvent=function(a,d){b.log(d)};this.insertImgWithThumb=
function(a,d){b("#wbbModalWindow").hide();if(q){var c=j.getTextWithInfo();g.value=g.value.substr(0,c.start)+bbstring+g.value.substr(c.end,g.value.length-c.end)}else{c=d&&i.allButtons.img.uploader.useThumb?u(m('<a href="{img}"><img src="{thumb}" /></a>',{img:a,thumb:d}),t):u(m('<img src="{img}" />',{img:a,thumb:d}),t);j.overrideWithNode(c,j.getSavedRange())}r()};i.smileAutoDetect&&ca();(function(){n.css("border","0").css("outline","none");n.after(m('<link rel="stylesheet" type="text/css" media="all" href="{themePrefix}/{themeName}/theme.css" />',
...
o),document));p.append(C);if(!o.isDefault){b.extend(f.htmlToBB,o.htmlToBB);if(!o.bbToHTML){o.bbToHTML={};var F=o.bbOpen.replace(/\s+/,"\\s")+"(.*?)"+o.bbClose.replace(/\s+/,"\\s");o.bbToHTML[F]=o.htmlOpen+"$1"+o.htmlClose}if(!f.bbToHTML)f.bbToHTML={};b.extend(f.bbToHTML,o.bbToHTML)}y.push({opt:o,el:C,option:z})}}i.allButtons[h]=f;f=new v(y,k,f);I.push(f);H.push(h);d.append(k);k.live("click",function(a){U(".wysibb-toolbar-select",".select-list",a)})}else if(k&&k=="colorpicker"){k=b(u(m('<div class="wysibb-toolbar-colorpicker" title="{title}">{buttonHTML}<span class="cpicker-dropdown"></span><div class="clist"><div class="nocolor" title="\u041d\u0435 \u0432\u044b\u0431\u0440\u0430\u043d">\u0410\u0432\u0442\u043e</div></div>',
f),document));p=k.find("div.clist");A=f.colorList?f.colorList.split(","):[];for(x=0;x<A.length;x++){z=b.trim(A[x])||"";A[x]=z;z=="-"?p.append('<span class="pl"></span>'):p.append(m('<div class="scolor" style="background:{color}" title="{color}"></div>',{color:z}))}f=new w(A,k,f);I.push(f);H.push(h);d.append(k);k.live("click",function(a){U(".wysibb-toolbar-colorpicker",".clist",a)})}else if(k&&k=="table"){k=b(u(m('<div class="wysibb-toolbar-table" title="{title}">{buttonHTML}<span class="cpicker-dropdown"></span><div class="clist"></div>',
f),document));p=k.find("div.clist");z=f.rows||10;A=f.cols||10;y=z*A;p.css("width",A*f.tdwidth+2+"px").css("height",z*f.tdwidth+2+"px");for(x=1;x<=A;x++)for(o=1;o<=z;o++)p.append(m('<div class="tbl-sel" style="width:{width}px;height:{height}px;z-index:{zindex}" title="{row},{col}"></div>',{width:x*f.tdwidth,height:o*f.tdwidth,zindex:--y,row:o,col:x}));f=new M(k,f);I.push(f);H.push(h);d.append(k);k.live("click",function(a){U(".wysibb-toolbar-table",".clist",a)})}else{f.buttonHTML=m(f.buttonHTML,i);
...
d.find(".btnModeSwitch").click(function(){if(q){q=false;B.html(T(n.val()));D.show();n.hide().val("");b(this).removeClass("on")}else{q=true;l&&(l.lastChild&&l.lastChild.nodeName.toLowerCase()=="br")&&b(l.lastChild).remove();n.val(R(l));D.hide();n.show();b(this).addClass("on")}})}if(i.smileList&&i.smileList.length>0){a=n.after('<div class="wysibb-toolbar wysibb-bottom-toolbar"></div>').next();for(c=0;c<i.smileList.length;c++){d=i.smileList[c];d.img=m(d.img,i);k=b(u(m('<div class="wysibb-toolbar-btn" title="{title}" unselectable="on">{img}</div>',
d),document));a.append(k);f=new s(new K.smileCommand,k,d)}a.append('<div class="wysibb-powered" title="\u0420\u0435\u0434\u0430\u043a\u0442\u043e\u0440 BBcode" unselectable="on"><a href="http://www.wysibb.com" target="_blank">WysiBB</a></div>')}da();n.wrap('<div class="wysibb-text"></div>');if(i.onlyBBmode!==true){c=n.width();k=n.outerHeight();n.css("width",c-12+"px").css("margin","0").css("font-size","14px").css("padding","0").css("max-width","100%");D=b(m('<iframe src="about:blank" class="wysibb-text-iframe" frameborder="0" style="margin:0;width:100%;height:{height}px;max-width:100%"></iframe>',
{width:c,height:k}));D.bind("load",function(){L=D.get(0);G=L.contentWindow;t=L.contentWindow.document;l=L.contentWindow.document.body;B=b(l);var a=t.getElementsByTagName("head")[0];b(document.getElementsByTagName("head")[0]).find("link[rel='stylesheet']").each(function(c,d){b(a).append(b(d).clone())});b(document.body).find("link[rel='stylesheet']").each(function(c,d){b(a).append(b(d).clone())});B.addClass("wysibb-body").addClass(i.bodyClass);if("contentEditable"in l&&!W){l.contentEditable=true;try{t.execCommand("StyleWithCSS",
false,false)}catch(c){}}else q=i.onlyBBmode=true;if(n.val()!=""){B.html(T(n.val()));n.val("");J()}W?b(t).live("click",r):b(t).live("mouseup",r);b(t).live("keyup",function(){r();J()});i.tabInsert==true&&b(t).live("keydown",function(a){if(a.which==9){a.preventDefault&&a.preventDefault();j.overrideWithNode('<span class="tab">\ufeff</span>',null)}});b(t).live("keydown",function(a){if(a.which==86&&(a.ctrlKey==true||a.metaKey==true)||a.which==45&&(a.shiftKey==true||a.metaKey==true)){j.saveRange();b(l).removeAttr("contentEditable");
var c=u("<div></div>",t);b(c).attr("contenteditable","true").attr("class","paste").appendTo(l).on("paste",function(){setTimeout(function(){b(c).text();ba(c);var a=b(c).html();b(c).remove();b(l).attr("contentEditable","true");l.focus();j.overrideWithNode("<span>"+a+"</span>",j.getSavedRange())},1)}).focus()}});if(i.watchTxtArea===true){b(document).live("click",function(a){if(!q&&b(a.target).parents(".wysibb").size()==0&&n.data("prevVal")!=n.val()){b.log("txtArea has changed");j.overrideWithNode(T(n.val()),
